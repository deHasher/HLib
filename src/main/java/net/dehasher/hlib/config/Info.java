...
public class Info extends Configuration {
	...
	@Comment({
			"",
			"Заголовки запросов в формате JSON.",
			"Используется в основном для авторизации по API.",
			"Задавать сюда настройки следует только если они были предоставлены лично ${author}'ом.",
			"Ключ JSON - полный URL запроса, значение - объект со строковыми заголовками.",
			"URL должен точно совпадать с адресом запроса после замены заполнителей.",
			"Заголовки, переданные в Informer явно, имеют приоритет.",
			"Пример: {\"https://example.com/api\":{\"Authorization\":\"Bearer EXAMPLE_KEY\",\"X-Client\":\"example\"}}",
	})
	@Key("headers")
